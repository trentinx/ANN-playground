...
class connection:
    ...
    def updt(self,lambd) :
        ...
        self.sumlam=(1-lambd)*self.sumlam+lambd*((math.sqrt(self.sumlam)+10**(-8))/(math.sqrt(self.sumgrad)+10**(-8))*grad)**2
        nrate=(self.sumlam+10**(-8))/(math.sqrt(self.sumgrad)+10**(-8))
...
class layered_network:
    ...
    def multilearn(self, Input, expected,lambd):
        err=[]
    ...
    def cyclelearn(self, Input, expected, nb,lambd):
        Err=[]
        for i in range(nb):
            Err.append(self.multilearn(Input, expected,lambd))
        return Err
...
if __name__ == "__main__":
    ...
    L = layered_network(len_in, len_out, 10, 10, 1, 1, 1, sigmoid, sigmoid)
    print("Avant apprentissage :")
    for i, n in enumerate(inputs):
        print(f"Input: {n}, Output: {L.getoutput(n)}, Expected: {out[i]}")
        input()

    listerr=L.cyclelearn(inputs, out,1000,0.6)  # Taux d'apprentissage réduit et 5000 itérations
